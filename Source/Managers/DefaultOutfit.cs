...
namespace BetterPawnControl
{
    public class DefaultOutfit : MapComponent
    {
        ...
        public override void MapComponentTick()
        {
            ...
            if (Current.Game.tickManager.TicksGame % 1800 == 0 && !Find.WindowStack.IsOpen<MainTabWindow_Assign_Policies>())
            {
                //Log.Message("[BPC] Tick! 600");
                //check if a new pawn has joined the player colonies
                IEnumerable<Pawn> Pawns = Find.VisibleMap.mapPawns.FreeColonists;
